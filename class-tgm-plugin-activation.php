...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
         ...
         * @var string
         */
        public $menu = 'plms-install-plugins';
         ...
         * does the plms_init action hook, and hooks in the interactions to init.
         *
         * @since 1.0.0
         *
        ...
        public function __construct() {
            ...
            $this->strings = array(
                'page_title'                     => __( 'Install Required Plugins', 'plms' ),
                'menu_title'                     => __( 'Install Plugins', 'plms' ),
                'installing'                     => __( 'Installing Plugin: %s', 'plms' ),
                'oops'                           => __( 'Something went wrong.', 'plms' ),
                ...
                'return'                         => __( 'Return to Required Plugins Installer', 'plms' ),
                'dashboard'                      => __( 'Return to the dashboard', 'plms' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'plms' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'plms' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'plms' ),
                'dismiss'                        => __( 'Dismiss this notice', 'plms' ),
            );
            ...
            do_action_ref_array( 'plms_init', array( $this ) );
         ...
         *
         * We get the links in the admin notices to point to the plms page, rather
         ...
         * @return null Returns early if not the plms page.
         */
        ...
        public function thickbox() {
            ...
            if ( ! get_user_meta( get_current_user_id(), 'plms_dismissed_notice', true ) ) {
                add_thickbox();
            }
        ...
        public function install_plugins_page() {
            ...
            $plugin_table = new plms_List_Table;

            // Return early if processing a plugin installation action.
            if ( isset( $_POST['action'] ) && 'plms-bulk-install' == $_POST['action'] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() ) {
                return;
            }
            ...
            <div class="plms wrap">
                ...
                <?php $plugin_table->plms_prepare_items(); ?>

                <?php if ( isset( $this->message ) ) {
                    echo wp_kses_post( $this->message );
                } ?>

                <form id="plms-plugins" action="" method="post">
                    <input type="hidden" name="plms-page" value="<?php echo $this->menu; ?>" />
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['plms-install'] ) && 'install-plugin' == $_GET['plms-install'] ) ) {
                check_admin_referer( 'plms-install' );
                ...
                $fields = array( 'plms-install' ); // Extra fields to pass to WP_Filesystem.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'plms' ) . '">' . __( 'Return to the Dashboard', 'plms' ) . '</a>' ) . '</p>';
            ...
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['plms-activate'] ) && 'activate-plugin' == $_GET['plms-activate'] ) ) {
                check_admin_referer( 'plms-activate', 'plms-activate-nonce' );
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'plms' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'plms' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'plms' ) . '</a>'  : '';

                // Define all of the action links.
                $action_links = apply_filters(
                    'plms_notice_action_links',
                    array(
                        'install'  => ( current_user_can( 'install_plugins' ) )  ? $show_install_link  : '',
                        'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
                        'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . esc_url( add_query_arg( 'plms-dismiss', 'dismiss_admin_notices' ) ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'plms', 'plms', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'plms', 'plms', $rendered, $nag_class );
                }
            }

            // Admin options pages already output settings_errors, so this is to avoid duplication.
            if ( 'options-general' !== $current_screen->parent_base ) {
                settings_errors( 'plms' );
            }
        ...
        public function dismiss() {
            ...
            if ( isset( $_GET['plms-dismiss'] ) ) {
                update_user_meta( get_current_user_id(), 'plms_dismissed_notice', 1 );
            }
        ...
        public function actions( $install_actions ) {
            ...
            // Remove action links on the plms install page.
            if ( $this->is_plms_page() ) {
                return false;
            }
        ...
        /**
         * Determine if we're on the plms Install page.
         *
         * @since 2.1.0
         *
         * @return boolean True when on the plms page, false otherwise.
         */
        protected function is_plms_page() {
        ...
        public function update_dismiss() {
            ...
            delete_user_meta( get_current_user_id(), 'plms_dismissed_notice' );
    ...
    $plms = TGM_Plugin_Activation::get_instance();

}

if ( ! function_exists( 'plms' ) ) {
    /**
     ...
     */
    function plms( $plugins, $config = array() ) {
...
if ( ! class_exists( 'plms_List_Table' ) ) {
    /**
     ...
     */
    class plms_List_Table extends WP_List_Table {

        /**
         * References parent constructor and sets defaults for class.
         *
         * The constructor also grabs a copy of $instance from the plms class
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'plms' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'plms' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'plms' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'plms' ) : __( 'Recommended', 'plms' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'plms' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'plms' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Install', 'plms' ) . ' %2$s">' . __( 'Install', 'plms' ) . '</a>',
                        esc_url(
                            wp_nonce_url(
                                add_query_arg(
                                    array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Activate', 'plms' ) . ' %2$s">' . __( 'Activate', 'plms' ) . '</a>',
                        esc_url(
                            add_query_arg(
                                array(
                                    'page'                 => urlencode( TGM_Plugin_Activation::$instance->menu ),
                                    ...
                                    'plugin_name'          => urlencode( $item['sanitized_plugin'] ),
                                    'plugin_source'        => urlencode( $item['url'] ),
                                    'plms-activate'       => 'activate-plugin',
                                    'plms-activate-nonce' => urlencode( wp_create_nonce( 'plms-activate' ) ),
                                ),
                                network_admin_url( 'themes.php' )
                            )
                        ),
        ...
        public function column_cb( $item ) {
            ...
            if ( __( 'Private Repository', 'plms' ) === $item['source'] ) {
                // Escape external URLs
                $plugin_url = esc_url( $plugin_url );
            } elseif ( __( 'Pre-Packaged', 'plms' ) === $item['source'] ) {
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'plms' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'plms' ),
                'source' => __( 'Source', 'plms' ),
                'type'   => __( 'Type', 'plms' ),
                'status' => __( 'Status', 'plms' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'plms-bulk-install'  => __( 'Install', 'plms' ),
                'plms-bulk-activate' => __( 'Activate', 'plms' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'plms-bulk-install' === $this->current_action() ) {
                ...
                // Loop through each plugin to install and try to grab information from WordPress API, if not create 'plms-empty' scalar.
                $i = 0;
                foreach ( $plugin_installs as $plugin ) {
                    $api[$i] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$i] = 'plms-empty';
                    $i++;
                }
            ...
            if ( 'plms-bulk-activate' === $this->current_action() ) {
                check_admin_referer( 'bulk-' . $this->_args['plural'] );
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'plms' ), $imploded );
                }
         ...
         * @since 2.2.0
         */
        public function plms_prepare_items() {
...
add_action( 'admin_init', 'plms_load_bulk_installer' );
if ( ! function_exists( 'plms_load_bulk_installer' ) ) {
    function plms_load_bulk_installer() {
    /**
    ...
    if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET['page'] ) && TGM_Plugin_Activation::$instance->menu === $_GET['page'] ) ) {
        ...
        if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
            ...
            class TGM_Bulk_Installer extends WP_Upgrader {
                 ...
                 * set to true for the plms class.
                 *
                 * @since 2.2.0
                 *
                ...
                public function run( $options ) {
                    ...
                    public function install_strings() {
                        ...
                        $this->strings['no_package']          = __( 'Install package not available.', 'plms' );
                        $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'plms' );
                        $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'plms' );
                        $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'plms' );
                        $this->strings['process_failed']      = __( 'Plugin install failed.', 'plms' );
                        $this->strings['process_success']     = __( 'Plugin installed successfully.', 'plms' );
                    ...
                    public function activate_strings() {
                        ...
                        $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'plms' );
                        $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'plms' );
            ...
            if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
                ...
                class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                    ...
                    public function add_strings() {
                        ...
                        if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                            $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'plms' );
                            $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'plms' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'plms' ) . '</span><span class="hidden">' . __( 'Hide Details', 'plms' ) . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'plms' );
                            $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'plms' );
                        }
                        // Default installation strings.
                        else {
                            $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'plms' );
                            $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'plms' );
                            $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'plms' );
                            $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'plms' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'plms' ) . '</span><span class="hidden">' . __( 'Hide Details', 'plms' ) . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'plms' );
                            $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'plms' );
                        }
                    ...
                    public function bulk_footer() {
                        ...
                        if ( empty( $complete ) ) {
                            echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'plms' ) . '">' . __( 'Return to the Dashboard', 'plms' ) . '</a>' ) . '</p>';
                            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                        }
